...
class StackAllocator : public StackAllocatorBase
{
  private:
    ...
    template <typename T>
    struct AllocationPtr : public PtrReturn<T>
    {
        AllocationHeader header;

        operator AllocationPtrReturn<void>() const noexcept
        {
            return {.ptr = this->ptr, .header = AllocationHeader(header.startOffset, header.startOffset)};
        }
    };

    template <typename T>
    struct ArrayPtr : public PtrReturn<T>
    {
        ArrayHeader header;

        operator ArrayPtrReturn<void>() const noexcept
        {
            return {.ptr = this->ptr, .header = ArrayHeader(header.startOffset, header.startOffset, header.count)};
        }
    };

    template <typename T>
    using Ptr = std::conditional<allocatorPolicy.HeaderPolicy == StackCheckPolicy::None, T*, ReturnPtr<T>>::type;

    template <typename T>
    using ArrayPtr = std::conditional<allocatorPolicy.HeaderPolicy == StackCheckPolicy::None, T*, ReturnArrayPtr<T>>::type;
